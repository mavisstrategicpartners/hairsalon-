...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="relative flex h-full flex-col border border-[#c9a84c]/30 bg-[#111111] px-5 pb-5 pt-6">
      {product.kind === 'service' ? (
        <span className="absolute left-5 top-5 z-10 rounded-full border border-[#c9a84c] bg-[#070707] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-[#c9a84c]">
          Service
        </span>
      ) : null}
      ...
      <button
        ...
        className="absolute right-5 top-5 z-10 text-[#c9a84c] transition-colors hover:text-[#e8c97a]"
      >
        <Heart className={`h-5 w-5 ${loved ? 'fill-[#c9a84c] text-[#c9a84c]' : ''}`} />
      </button>

      <Link href={`/product/${product.slug}`} className="mx-auto mt-4 block">
        <span className="relative mx-auto block h-44 w-44 overflow-hidden rounded-full bg-[#070707] ring-1 ring-[#c9a84c]/40">
          <Image
            src={product.image}
            alt={product.name}
            fill
      ...
      <div className="mt-6 flex flex-1 flex-col">
        <Link href={`/product/${product.slug}`}>
          <h3 className="font-display text-xl italic leading-snug tracking-tight text-[#f4ead8]">
            {product.name}
          </h3>
        </Link>
        {product.length ? (
        ...
        <button
          ...
          className="mt-4 w-full border-2 border-[#c9a84c] bg-[#c9a84c] py-2.5 text-[0.68rem] font-bold uppercase tracking-[0.16em] text-[#070707] transition-colors hover:bg-[#e8c97a] hover:border-[#e8c97a]"
        >
          Add to bag
        </button>
      </div>
